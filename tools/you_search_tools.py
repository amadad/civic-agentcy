...
def you_search_wrapper(query: str, fetch_raw: bool = False, num_results: int = 1) -> str:
    """
...
def you_fetch_raw(query: str) -> str:
    ...
    """
    utility = YouSearchAPIWrapper(num_web_results=10)  # Example: Fetching 10 results
...
def you_news_search(query: str) -> str:
    ...
    search_results = response.json()['news']['results']  # Adjusted to match the given schema
    summarized_results = []
    ...
    for result_item in search_results:
        ...
        summarized_results.append({
            ...
            'original_description': result_item['description'],
            'summarized_text': summarized_text,
            'url': result_item['url'],
            'thumbnail': result_item['thumbnail']['original']
        })
